...
cuda_module = load(
    ...
    sources=["fused_grouped_tril_mask_softmax_ops.cpp", "kernel/fused_grouped_tril_mask_softmax_kernel.cu"],
    verbose=True,
)
...
def cuda_test(attn):
    ...
    attn = cuda_module.torch_launch_fused_grouped_tril_mask_softmax(attn, seq_lens)
    print(attn)
    return attn
...
print(torch.max(torch.abs(torch_result - cuda_result)))
